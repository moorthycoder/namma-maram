...
function getCurrentRoleType() { var urlRole=new URLSearchParams(location.search).get('role'); var userid_q=new URLSearchParams(location.search).get('userid'); if(urlRole){ var r=String(urlRole).toLowerCase(); return r.indexOf('sponsor')===0||r.indexOf('spn')===0?'sponsor':r.indexOf('care')===0||r.indexOf('car')===0?'caregiver':r; } if(userid_q){ try{ var l=storage.get('login')||window._login||{}; var tl=l['tree-login']||{}; for(var k in tl) if(tl[k]&&tl[k].userId===userid_q){ var t=tl[k].type||k; return t==='sponsor'||k==='sponsor'?'sponsor':'caregiver'; } var s=sessionStorage.getItem('loginCredentialsV1'); if(s){ var c=JSON.parse(s); var tl2=c['tree-login']||{}; for(var k2 in tl2) if(tl2[k2]&&tl2[k2].userId===userid_q){ var t2=tl2[k2].type||k2; return t2==='sponsor'||k2==='sponsor'?'sponsor':'caregiver'; } } }catch(e){} } return null; }
function isOpenedFromDashboardList(role){ var sponsorArr=['sponsor-waiting-submitted','sponsor-current','sponsor-past','sponsor-next-due','sponsor-seeing','sponsor-dash','pay-logs-total','pay-logs-this_month','pay-logs','tree-logs']; var caregiverArr=['caregiver-waiting','caregiver-current','caregiver-past','caregiver-seeing','caregiver-dash','caregiver-checks-due','caregiver-checks-finished','caregiver-logs-approved','caregiver-logs-submitted','tree-logs']; var excludeArr=role==='caregiver' ? [caregiverArr] : role==='sponsor' ? [sponsorArr] : [sponsorArr,caregiverArr]; var parent=new URLSearchParams(location.search).get('parent')||''; try{ var goback=sessionStorage.getItem('gobackFromTreeProfile')||''; if(goback) parent+='|'+goback; }catch(e){} for(var i=0;i<excludeArr.length;i++) for(var j=0;j<excludeArr[i].length;j++) if(parent.indexOf(excludeArr[i][j])!==-1) return true; return false; }
function applyRoleVisibility() { var role=getCurrentRoleType(); var showCare=true; var showSponsor=true; if(role==='sponsor'){ showCare=false; showSponsor=true; } else if(role==='caregiver'){ showCare=true; showSponsor=false; } var careBtns=document.querySelectorAll('.add-care-btn, .care-cta'); var sponsorBtns=document.querySelectorAll('.add-sponsor-btn, .sponsor-cta'); console.log('[tree-profile] role ->', role); careBtns.forEach(function(el){ el.style.display=showCare?'':'none'; }); sponsorBtns.forEach(function(el){ el.style.display=showSponsor?'':'none'; }); if(isOpenedFromDashboardList(role)){ if(role==='caregiver'){ careBtns.forEach(function(el){ el.style.display='none'; }); } else if(role==='sponsor'){ sponsorBtns.forEach(function(el){ el.style.display='none'; }); } else { sponsorBtns.forEach(function(el){ el.style.display='none'; }); careBtns.forEach(function(el){ el.style.display='none'; }); } } }
