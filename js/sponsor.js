...
function loadDashboard() {
  ...
  if (legacy_next_due_id_el) {
    ...
    legacy_next_due_id_el.classList.toggle('hidden', !window._sponsorNextDueId);
  }
...
function openSponsorNextDue() {
  ...
  if (!sponsor_next_due_list_local.length) {
    try {
      var lg2 = storage.get('login') || window.__login || {};
...
function renderSponsorNextDueSingleCard(single_tid) {
  ...
  if (emptyEl) emptyEl.classList.add('hidden'); if (cardEl) cardEl.innerHTML = sponsorNextDueCardHtml(c);
  return true;
}
...
function loadListForDashBoardBtns(btn_name) {
  ...
  if (empty_el) empty_el.classList.toggle('hidden', tree_list.length > 0);
...
function openSponsorWaitingSubmittedRequests() {
  var titleEl = document.getElementById('swaiting-submitted-page-title');
  if (titleEl) titleEl.textContent = 'New requests';
  var role = (window.__login && window.__login['tree-login'] && window.__login['tree-login']['sponsor']) || {};
...
function renderSponsorCards() {
  ...
  if (waitingSubmittedEmpty) waitingSubmittedEmpty.classList.toggle('hidden', waiting_submitted_list.length > 0);
...
function sponsorSeeingCardHtml(t) {
  ...
  var name_txt = sponsorCardName(t);
  var addr_txt = sponsorCardAddr(t);
  var login_for_pay = storage.get('login') || window.__login || {};
  ...
  return '<div class="sponsor-tree-card">'
    ...
    + '<div class="s-metric-row"><div class="s-metric-col"><span class="s-metric-label">Logs</span><div class="s-metric-val-row"><span class="sponsor-underline" onclick="event.stopPropagation();openTreeLogs(' + q + 'sponsor-seeing' + q + ',' + q + t.treeId + q + ')">' + logs_display + '</span></div></div><div class="s-metric-divider"></div><div class="s-metric-col"><span class="s-metric-label">Total paid</span><div class="s-metric-val-row"><span class="sponsor-underline" onclick="paylogTree(\'' + t.treeId + '\',\'sponsor-seeing\')">' + paid_display + '</span></div></div></div>'
...
function consumePendingSponsorRequest() {
  try {
    ...
    if (changed) { login['tree-login'] = tl; storage.set('login', login); window.__login = login; console.log('[sponsor] saved login', login); }
...
function openTreePool() {
  var login = storage.get('login') || window.__login || {};
  var role = (login['tree-login'] && login['tree-login']['sponsor']) || (window.__login && window.__login['tree-login'] && window.__login['tree-login']['sponsor']) || {};
