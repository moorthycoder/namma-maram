...
function openRegisterATreePage() {
  ...
  var parent_url = encodeURIComponent('surveyor.html?hub=' + current_hub + '&scroll=' + scroll_pos);
  var user_id = surveyor_role.userId ? surveyor_role.userId : '';
...
function requestTreeForSurvey() {
  ...
  try { sessionStorage.setItem('hubScrollTop', String(scroll_pos)); } catch (e) {}
  var current_hub = active_el ? active_el.id.replace('page-','') : 'surveyor-dash';
  var parent_param = encodeURIComponent('surveyor.html?hub=' + current_hub + '&scroll=' + scroll_pos);
  var user_id = surveyor_role.userId ? surveyor_role.userId : '';
...
function confirmDeleteSurveyorCard() {
  ...
  if (type === 'tree') {
    ...
    try { var login_data = window.__login || storage.get('login') || {}; login_data['tree-login'] = login_data['tree-login'] || {}; login_data['tree-login']['surveyor'] = role_cfg; storage.set('login', login_data); window.__login = login_data; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
    ...
    try { var login_data2 = window.__login || storage.get('login') || {}; login_data2['tree-login'] = login_data2['tree-login'] || {}; login_data2['tree-login']['surveyor'] = role_cfg2; storage.set('login', login_data2); window.__login = login_data2; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
    ...
    try { var login_data3 = window.__login || storage.get('login') || {}; login_data3['tree-login'] = login_data3['tree-login'] || {}; login_data3['tree-login']['surveyor'] = role_cfg3; storage.set('login', login_data3); window.__login = login_data3; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
    var active_el = document.querySelector('.page.active');
    ...
    try { var login_data4 = window.__login || storage.get('login') || {}; login_data4['tree-login'] = login_data4['tree-login'] || {}; login_data4['tree-login']['surveyor'] = role_cfg4; storage.set('login', login_data4); window.__login = login_data4; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
    ...
    try { var login_data5 = window.__login || storage.get('login') || {}; login_data5['tree-login'] = login_data5['tree-login'] || {}; login_data5['tree-login']['surveyor'] = role_cfg5; storage.set('login', login_data5); window.__login = login_data5; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
    ...
    try { var login_data6 = window.__login || storage.get('login') || {}; login_data6['tree-login'] = login_data6['tree-login'] || {}; login_data6['tree-login']['surveyor'] = role_cfg6; storage.set('login', login_data6); window.__login = login_data6; } catch (e) {}
    try { renderSurveyorStats(); } catch (e) {}
...
function surveyorLogCardHtml(t, loggedAt, hideLogBox, target_id) {
  ...
  var is_survey_log = target_str.indexOf('surveyor-logs') > -1 || target_str.indexOf('this-month-log') > -1 ? true : false;
  ...
  if (is_survey_log) {
    ...
    var survey_header = is_submitted ? '<div class="surveyor-log-header"><span></span>' + delete_btn + '</div>' : '';
    return '<div class="log-entry log-entry-interactive">' + survey_header + '<div class="log-entry-row" onclick="openSurveyorReviewPage(' + q + t.treeId + q + ',' + q + at + q + ')"><div class="log-dot log-dot-green"></div><div class="log-body"><div class="log-date">' + at + '</div><div class="log-text">' + name_txt + ' · ' + t.treeId + '</div><div class="log-addr">' + addr_txt + '</div><div class="log-text">Health: ' + (st.health || '—') + '</div><div class="log-text">Height: ' + (formatLength(st.height,'height') || '—') + '</div><div class="log-text">Diameter: ' + (formatLength(st.diameter,'diameter') || '—') + '</div><div class="log-text"><span class="log-label-danger">Note:</span> ' + note_txt + '</div><div class="log-text"><span class="log-label-danger">Recommendation:</span> ' + rec_txt + '</div><div class="log-text"><span class="chip-blue" onclick="event.stopPropagation();openSurveyorReviewPage(' + q + t.treeId + q + ',' + q + at + q + ')"><i class="ti ti-eye"></i> Review</span></div></div></div></div>';
  }
...
function openSurveyorSurveyCurrent() {
  var role_cfg = getRoleConfig('surveyor');
  var tiles_my = role_cfg.tiles && role_cfg.tiles['my-trees'] || {};
  var stats_my = role_cfg.stats && role_cfg.stats['my-trees'] || {};
  var current_list = Array.isArray(tiles_my.current) ? tiles_my.current.slice() : (Array.isArray(stats_my.current) ? stats_my.current.slice() : []);
  ...
  var map = {}; current_list.forEach(function(e){ if(e && e.treeId) map[e.treeId]=e.addedAt||''; });
  renderSurveyorMyTreeCards('surveyor-my-current-cards', 'surveyor-my-current-empty', current_ids, map);
  var target_el = document.getElementById('surveyor-my-current-cards');
  if (target_el) { target_el.innerHTML = target_el.innerHTML.split('openProfile(').join('surveyorSurveyTree('); }
  var title_el = document.querySelector('#page-surveyor-my-current .topbar-title');
...
if (hubMode === 'login') { window.location.href = 'login-hub.html?role=surveyor'; }
...
function loginCheckSurveyor() {
  var _r = window.__login || null;
  if (_r && _r['tree-login'] && _r['tree-login'].surveyor && _r['tree-login'].surveyor.loggedIn) {
    return true;
  }
  window.location.href = 'login-hub.html?role=surveyor';
  return false;
}
if (hubMode !== 'login' && hubMode !== 'register') { loginCheckSurveyor(); }
