...
function openProfile(treeId) {
  ...
  var parent_url = encodeURIComponent('ten-trees-ranger.html?hub=' + current_hub + '&scroll=' + scroll_pos);
...
function renderTenTreesRangerNextDueSingleCard(single_tid) {
  ...
  if (emptyEl) emptyEl.classList.add('hidden'); if (cardEl) cardEl.innerHTML = tenTreesRangerCardHtml(c);
  return true;
}
...
window.render = {
  init: function () {
    ...
    if (hubMode === 'ten-trees-ranger-logs-approved') { goTo('ten-trees-ranger-logs-approved'); renderTenTreesRangerLogs('approved'); }
    if (hubMode === 'ten-trees-ranger-logs-submitted') { goTo('ten-trees-ranger-logs-submitted'); renderTenTreesRangerLogs('submitted'); }
...
function openTreePool() {
  ...
  console.log('[ten-trees-ranger] openTreePool ->', target_url);
