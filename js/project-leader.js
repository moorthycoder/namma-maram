...
function loadCurrentUser() {
  try {
    ...
    var role = cred['tree-login'] && cred['tree-login']['project-leader'];
    if (!role) { return; }
...
var registerStatusData = {
  ...
  existing_member:  { icon:'ti ti-user-check',  color:'#16a34a', bg:'#dcfce7', title:'Already registered',        text:'An account with this email already exists. Please log in instead of registering again.', go:'Login', to:'project-leader-login' },
  blocked:          { icon:'ti ti-ban',         color:'#dc2626', bg:'#fee2e2', title:'Registration blocked',       text:'Your registration has been blocked. Please contact support if you think this is a mistake.', go:'Contact us', to:'project-leader-login' },
...
var regTarget = 'project-leader-login';
...
document.addEventListener('click', function(e) {
  ...
  var lsd = document.getElementById('logout-drop-project-leader');
...
function toggleProjectLeaderLogoutDrop() {
  document.getElementById('logout-drop-project-leader').classList.toggle('open');
}
function projectLeaderLogout() {
  try {
...
function switchProjectLeaderPanel(snake_case) {
  var is_action = snake_case === 'action';
  var action_panel = document.getElementById('panel-project-leader-action');
  var stat_panel = document.getElementById('panel-project-leader-stat');
...
function goBackToProjectLeaderStats() {
...
function openAppendTreeName() {
  ...
  frame.src = 'append-a-tree-species-name.html?parent=' + encodeURIComponent('project-leader.html?hub=project-leader-tree-name-submitted') + '&userid=' + encodeURIComponent(user_id);
...
function openAppendPlaceName() {
  ...
  frame.src = 'append-a-place-name.html?parent=' + encodeURIComponent('project-leader.html?hub=project-leader-place-name-submitted') + '&userid=' + encodeURIComponent(user_id);
  var dash_page = document.getElementById('page-project-leader-dash');
  if (dash_page) dash_page.style.display = 'none';
  var screen = document.querySelector('.screen');
  if (screen) screen.insertBefore(frame, dash_page ? dash_page.nextSibling : null);
...
function requestTreeForSurvey() {
  ...
  var project_leader_cards = project_leader_role.cards || {};
  ...
  var project_leader_waiting = [];
  try { project_leader_waiting = JSON.parse(sessionStorage.getItem('projectLeaderSurveyWaiting') || '[]'); } catch (e) {}
  var exclude_list = [].concat(
    (tiles_my.current || []).map(function (c) { return c.treeId || c; }),
    (tiles_my.past || []).map(function (c) { return c.treeId || c; }),
    (project_leader_cards.current || []).map(function (c) { return c.treeId || c; }),
    (project_leader_cards.past || []).map(function (c) { return c.treeId || c; }),
...
function getCurrentAddedAtStringProjectLeader() {
...
function projectLeaderRequestATree(form_data) {
  ...
  if (can_push) submitted_list.push({ treeId: tree_id, loggedAt: getCurrentAddedAtStringProjectLeader() });
  stats['survey-requests'].submitted = submitted_list;
  try { storage.set('login', login_data); } catch (e) {}
  if (typeof renderProjectLeaderStats === 'function') renderProjectLeaderStats();
  if (typeof openProjectLeaderSurveyRequests === 'function') setTimeout(function () { openProjectLeaderSurveyRequests('submitted'); }, 500);
}

function checkNewProjectLeaderTrees() {
  var waiting_str = sessionStorage.getItem('projectLeaderSurveyWaiting');
...
function backToStart() {
  ...
  if (frame) {
    try {
      ...
      if (parent_param && parent_param.indexOf('project-leader.html') > -1) { return_to = parent_param; }
    } catch (e) {}
  }
  if (frame && frame.parentNode) { frame.parentNode.removeChild(frame); }
  var dash_page = document.getElementById('page-project-leader-dash');
  if (dash_page) dash_page.style.display = '';
  if (return_to && return_to.indexOf('hub=project-leader-tree-name') > -1) {
    try { renderProjectLeaderStats(); } catch (e) {}
    if (return_to.indexOf('submitted') > -1) openProjectLeaderTreeName('submitted'); else openProjectLeaderTreeName('approved');
    return;
  }
  if (return_to && return_to.indexOf('hub=project-leader-place-name') > -1) {
    try { renderProjectLeaderStats(); } catch (e) {}
...
function treeLoc(t) { return projectLeaderCardAddr(t); }
...
function renderAlbum(place, tree) {
  ...
  var filtered = albumData.filter(function(t) {
    ...
    var matchTree = true;
    if (place) {
      matchPlace = q(projectLeaderCardAddr(t)).indexOf(place) > -1 || q(t.placeName || '').indexOf(place) > -1 || q(t.pincode || '').indexOf(place) > -1 || q(Array.isArray(t.projectName) ? t.projectName.join(', ') : (t.projectName || '')).indexOf(place) > -1;
    }
    if (tree) {
      matchTree = q(t.scientificName).indexOf(tree) > -1 ||
                  q(projectLeaderCardName(t) || '').indexOf(tree) > -1;
    }
  ...
  if (summaryEl) {
    if (filtered.length > 0) {
      ...
      filtered.forEach(function(t) {
        var gname = projectLeaderCardName(t) || t.scientificName || '—';
  ...
  filtered.forEach(function(t) {
    ...
    info.innerHTML = 
      ...
      '<div class="tree-name">' + (projectLeaderCardName(t) || t.scientificName || '') + '</div>' +
...
function renderProjectLeaderStats() {
  ...
  setStatById('pld-logs-approved', approved_val != null ? approved_val : '—');
  setStatById('pld-logs-submitted', submitted_val != null ? submitted_val : '—');
  ...
  setStatById('pld-tree-name-approved', tree_name_approved != null ? tree_name_approved : 0);
  setStatById('pld-tree-name-submitted', tree_name_submitted != null ? tree_name_submitted : 0);
  ...
  setStatById('pld-register-approved', register_approved != null ? register_approved : 0);
  setStatById('pld-register-submitted', register_submitted != null ? register_submitted : 0);
  ...
  setStatById('pld-survey-requests-approved', survey_req_approved != null ? survey_req_approved : 0);
  setStatById('pld-survey-requests-submitted', survey_req_submitted != null ? survey_req_submitted : 0);
  var month = stats.month || {};
  var c = (typeof countRoleLogs === 'function') ? countRoleLogs('project-leader') : { total: null, month: null, covered: null };
  ...
  if (typeof roleTreeIds === 'function' && typeof storage !== 'undefined' && storage.pullTreeDetail) {
    try {
      ...
      var ids = roleTreeIds('project-leader');
      var month_set = {};
      var month_log_cnt = 0;
  ...
  setStatById('pld-month-trees', month_trees != null ? month_trees : '—');
  setStatById('pld-month-logs', month_logs != null ? month_logs : '—');
  ...
  setStatById('pld-this-month-log-approved', this_month_log_approved);
  setStatById('pld-this-month-log-submitted', this_month_log_submitted);
  ...
  setStatById('pld-covered', covered_val != null ? covered_val : 0);
  setStatById('pld-waiting', waiting_len != null ? waiting_len : 0);
  setStatById('pld-my-current', my_current_len);
  setStatById('pld-my-past', my_past_len);
  setStatById('pld-my-trees-current', my_current_len);
  setStatById('pld-my-trees-past', my_past_len);
  setStatById('pld-zone-place', zone.place || '—');
  setStatById('pld-zone-future', zone.future != null ? zone.future : '—');
}

function openProjectLeaderLogsApproved() {
  var stats = getRoleConfig('project-leader').stats || {};
  var logs = stats["survey-log"] || {};
  ...
  renderProjectLeaderLogCards('project-leader-logs-approved-cards', 'project-leader-logs-approved-empty', approved_ids, map);
  goTo('project-leader-logs-approved');
}

function openProjectLeaderLogsSubmitted() {
  var stats = getRoleConfig('project-leader').stats || {};
  var logs = stats["survey-log"] || {};
  ...
  renderProjectLeaderLogCards('project-leader-logs-submitted-cards', 'project-leader-logs-submitted-empty', submitted_ids, map);
  goTo('project-leader-logs-submitted');
}
...
function projectLeaderTreeNameCardHtml(entry, is_submitted) {
  ...
  var q=String.fromCharCode(39);
  var delete_btn = is_submitted ? '<button class="tcard-delete-btn" type="button" onclick="event.stopPropagation(); deleteProjectLeaderTreeName('+q+sci.replace(/\'/g,"\\'")+q+')"><i class="ti ti-trash"></i></button>' : '';
  var header_html = is_submitted ? '<div class="project-leader-card-header"><span class="project-leader-card-header-title">Tree name request</span>' + delete_btn + '</div>' : '';
  return '<div class="info-card project-leader-card project-leader-clickable" onclick="openTreeNameEditForm('+q+sci.replace(/\'/g,"\\'")+q+')">' + header_html + '<div class="info-row"><span class="info-key"><i class="ti ti-leaf field-icon-sm"></i>Scientific</span><span class="info-val project-leader-val-strong">' + sci + '</span></div>' +
...
var pending_project_leader_delete_type = '';
var pending_project_leader_delete_key = '';
function deleteProjectLeaderTreeName(sci_name) {
  var sci = sci_name || '';
  pending_project_leader_delete_type = 'tree';
  pending_project_leader_delete_key = sci;
...
function confirmDeleteProjectLeaderCard() {
  var modal_el = document.getElementById('delete-confirm-modal');
  if (modal_el) modal_el.classList.remove('open');
  var type = pending_project_leader_delete_type;
  var key = pending_project_leader_delete_key;
  pending_project_leader_delete_type = '';
  pending_project_leader_delete_key = '';
  if (type === 'tree') {
    var sci = key;
    var role_cfg = getRoleConfig('project-leader');
    ...
    var empty_el = document.getElementById('project-leader-tree-name-submitted-empty');
    if (empty_el) empty_el.style.display = filtered_list.length ? 'none' : 'block';
  } else if (type === 'place') {
    var pin = key;
    var role_cfg2 = getRoleConfig('project-leader');
    ...
    if (is_approved_page) {
      ...
      renderProjectLeaderLogCards('project-leader-survey-requests-approved-cards', 'project-leader-survey-requests-approved-empty', idsA, mapA);
    } else {
      ...
      renderProjectLeaderLogCards('project-leader-survey-requests-submitted-cards', 'project-leader-survey-requests-submitted-empty', ids3, map3);
    }
  } else if (type === 'register') {
    var tid_reg = key;
    var role_cfg4 = getRoleConfig('project-leader');
    ...
    renderProjectLeaderLogCards('project-leader-register-log-submitted-cards', 'project-leader-register-log-submitted-empty', ids4, map4, true);
  } else if (type === 'mycurrent') {
    var tid_my = key;
    var role_cfg5 = getRoleConfig('project-leader');
    ...
    renderProjectLeaderMyTreeCards('project-leader-my-current-cards', 'project-leader-my-current-empty', ids5, map5);
  } else if (type === 'log') {
    var tid_log = key;
    var role_cfg6 = getRoleConfig('project-leader');
    ...
    renderProjectLeaderLogCards('project-leader-logs-submitted-cards', 'project-leader-logs-submitted-empty', ids6, map6);
  }
}
function cancelDeleteProjectLeaderCard() {
  pending_project_leader_delete_type = '';
  pending_project_leader_delete_key = '';
  var modal_el = document.getElementById('delete-confirm-modal');
  if (modal_el) modal_el.classList.remove('open');
...
function projectLeaderPlaceNameCardHtml(entry, is_submitted) {
  ...
  var label = dm ? dm[3] + '-' + dm[2] + '-' + dm[1] : revised;
  var q=String.fromCharCode(39);
  var delete_btn = is_submitted ? '<button class="tcard-delete-btn" type="button" onclick="event.stopPropagation(); deleteProjectLeaderPlaceName('+q+pin.replace(/\'/g,"\\'")+q+')"><i class="ti ti-trash"></i></button>' : '';
  var header_html = is_submitted ? '<div class="project-leader-card-header"><span class="project-leader-card-header-title">Place name request</span>' + delete_btn + '</div>' : '';
  return '<div class="info-card project-leader-card project-leader-clickable" onclick="openPlaceNameEditForm('+q+pin.replace(/\'/g,"\\'")+q+')">' + header_html + '<div class="info-row"><span class="info-key"><i class="ti ti-map-pin field-icon-sm"></i>Pin</span><span class="info-val">' + pin + '</span></div>' +
    '<div class="info-row"><span class="info-key"><i class="ti ti-language field-icon-sm"></i>EN</span><span class="info-val">' + en + '</span></div>' +
    (ta ? '<div class="info-row"><span class="info-key"><i class="ti ti-language field-icon-sm"></i>TA</span><span class="info-val">' + ta + '</span></div>' : '') +
    '<div class="info-row"><span class="info-key"><i class="ti ti-calendar field-icon-sm"></i>Revised</span><span class="info-val">' + label + '</span></div>' +
    '<div class="info-row"><span class="info-key"><i class="ti ti-check field-icon-sm"></i>Status</span><span class="info-val">' + status + '</span></div>' +
...
function deleteProjectLeaderPlaceName(pin_code) {
  var pin = pin_code || '';
  pending_project_leader_delete_type = 'place';
  pending_project_leader_delete_key = pin;
  var text_el = document.getElementById('delete-confirm-text');
...
function openProjectLeaderTreeName(status) {
  ...
  var target = (status === 'submitted') ? 'project-leader-tree-name-submitted-cards' : 'project-leader-tree-name-approved-cards';
  var empty = (status === 'submitted') ? 'project-leader-tree-name-submitted-empty' : 'project-leader-tree-name-approved-empty';
  var page = (status === 'submitted') ? 'project-leader-tree-name-submitted' : 'project-leader-tree-name-approved';
  renderProjectLeaderSimpleCards(target, empty, list);
  goTo(page);
}
function openProjectLeaderPlaceName(status) {
  var stats = getRoleConfig('project-leader').stats || {};
  var place_name = stats['place-name'] || {};
  var list = (status === 'submitted') ? (place_name.submitted || []) : (place_name.approved || []);
  var target = (status === 'submitted') ? 'project-leader-place-name-submitted-cards' : 'project-leader-place-name-approved-cards';
  var empty = (status === 'submitted') ? 'project-leader-place-name-submitted-empty' : 'project-leader-place-name-approved-empty';
  var page = (status === 'submitted') ? 'project-leader-place-name-submitted' : 'project-leader-place-name-approved';
  renderProjectLeaderSimpleCards(target, empty, list);
  goTo(page);
}
function openProjectLeaderRegisterLog(status) {
  var stats = getRoleConfig('project-leader').stats || {};
  ...
  var target = (status === 'submitted') ? 'project-leader-register-log-submitted-cards' : 'project-leader-register-log-approved-cards';
  var empty = (status === 'submitted') ? 'project-leader-register-log-submitted-empty' : 'project-leader-register-log-approved-empty';
  var hide = (status === 'submitted');
  renderProjectLeaderLogCards(target, empty, ids, map, hide);
  goTo((status === 'submitted') ? 'project-leader-register-log-submitted' : 'project-leader-register-log-approved');
}
function openProjectLeaderSurveyRequests(status) {
  var stats = getRoleConfig('project-leader').stats || {};
  ...
  var ids = list.map(function (e) { return e.treeId || e; }).filter(Boolean);
  var map = {}; list.forEach(function (e) { if (e.treeId) map[e.treeId] = e.loggedAt || ''; });
  var target = (status === 'submitted') ? 'project-leader-survey-requests-submitted-cards' : 'project-leader-survey-requests-approved-cards';
  var empty = (status === 'submitted') ? 'project-leader-survey-requests-submitted-empty' : 'project-leader-survey-requests-approved-empty';
  renderProjectLeaderLogCards(target, empty, ids, map);
  goTo((status === 'submitted') ? 'project-leader-survey-requests-submitted' : 'project-leader-survey-requests-approved');
}
function openThisMonthTree(status) {
  ...
  if (is_covered) {
    ...
    try {
      var now = new Date();
      var ym = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0');
      var role_ids = (typeof roleTreeIds === 'function') ? roleTreeIds('project-leader') : [];
      var month_set = {};
    ...
    renderProjectLeaderMyTreeCards(target, empty, ids, map);
  } else {
    var role_cfg2 = getRoleConfig('project-leader');
    ...
    if (!ids2.length) {
      var s_waiting = sessionStorage.getItem('projectLeaderSurveyWaiting');
      try { var arr = JSON.parse(s_waiting || '[]'); if (Array.isArray(arr)) ids2 = arr.slice(); } catch (e) {}
    }
    renderProjectLeaderMyTreeCards(target, empty, ids2, map2);
  }
  goTo(page);
}
...
function projectLeaderCardName(t) {
  if (!t) return '';
  if (typeof storage !== 'undefined' && storage.treeNameIn) { try { var lang_tmp = getProjectLeaderLang(); var res_tmp = storage.treeNameIn(t, lang_tmp); if (Array.isArray(res_tmp) && res_tmp.length) return res_tmp[0]; if (res_tmp) return res_tmp; } catch (e) {} }
  return t.scientificName || '';
}

function projectLeaderCardAddr(t) {
  if (!t) return '';
  var lang = getProjectLeaderLang();
...
function projectLeaderLogCardHtml(t, loggedAt, hideLogBox, target_id) {
  var q = String.fromCharCode(39);
  var c = t.card || {};
  var enc = t['encounters-list'] || {};
  var keys = Object.keys(enc);
  var last = enc[keys[keys.length - 1]] || {};
  var st = last['health-status'] || {};
  var name_txt = projectLeaderCardName(t) || '';
  var addr_txt = projectLeaderCardAddr(t) || c.addr || '';
  ...
  var is_survey_log = target_str.indexOf('project-leader-logs') > -1 || target_str.indexOf('this-month-log') > -1 ? true : false;
  var is_submitted = is_survey_requests ? true : target_str.indexOf('submitted') > -1 ? true : false;
  var delete_fn = is_survey_requests ? 'deleteProjectLeaderSurveyRequest' : is_register_log ? 'deleteProjectLeaderRegisterRequest' : is_survey_log ? 'deleteProjectLeaderLogRequest' : '';
  ...
  var header_html = (is_submitted && delete_fn) ? '<div class="project-leader-card-header"><span class="project-leader-card-header-title">' + header_title + '</span>' + delete_btn + '</div>' : '';
  if (is_survey_log) {
    ...
    var survey_header = is_submitted ? '<div class="project-leader-log-header"><span></span>' + delete_btn + '</div>' : '';
    return '<div class="log-entry log-entry-interactive">' + survey_header + '<div class="log-entry-row" onclick="openProjectLeaderReviewPage(' + q + t.treeId + q + ',' + q + at + q + ')"><div class="log-dot log-dot-green"></div><div class="log-body"><div class="log-date">' + at + '</div><div class="log-text">' + name_txt + ' · ' + t.treeId + '</div><div class="log-addr">' + addr_txt + '</div><div class="log-text">Health: ' + (st.health || '—') + '</div><div class="log-text">Height: ' + (formatLength(st.height,'height') || '—') + '</div><div class="log-text">Diameter: ' + (formatLength(st.diameter,'diameter') || '—') + '</div><div class="log-text"><span class="log-label-danger">Note:</span> ' + note_txt + '</div><div class="log-text"><span class="log-label-danger">Recommendation:</span> ' + rec_txt + '</div><div class="log-text"><span class="chip-blue" onclick="event.stopPropagation();openProjectLeaderReviewPage(' + q + t.treeId + q + ',' + q + at + q + ')"><i class="ti ti-eye"></i> Review</span></div></div></div></div>';
  }
  return '<div class="sponsor-tree-card" onclick="openProjectLeaderReviewPage(' + q + t.treeId + q + ',' + q + at + q + ')">' + header_html +
...
var pending_project_leader_survey_key = '';
function deleteProjectLeaderSurveyRequest(tree_id) {
  var tid = tree_id || '';
  pending_project_leader_delete_type = 'survey';
  pending_project_leader_delete_key = tid;
  var text_el = document.getElementById('delete-confirm-text');
  if (text_el) text_el.textContent = 'Remove survey request "' + tid + '"? This will remove it from your submitted list.';
  var modal_el = document.getElementById('delete-confirm-modal');
  if (modal_el) modal_el.classList.add('open');
}
function deleteProjectLeaderRegisterRequest(tree_id) {
  var tid = tree_id || '';
  pending_project_leader_delete_type = 'register';
  pending_project_leader_delete_key = tid;
  var text_el = document.getElementById('delete-confirm-text');
...
function deleteProjectLeaderMyCurrent(tree_id) {
  var tid = tree_id || '';
  pending_project_leader_delete_type = 'mycurrent';
  pending_project_leader_delete_key = tid;
  var text_el = document.getElementById('delete-confirm-text');
...
function deleteProjectLeaderLogRequest(tree_id) {
  var tid = tree_id || '';
  pending_project_leader_delete_type = 'log';
  pending_project_leader_delete_key = tid;
  var text_el = document.getElementById('delete-confirm-text');
...
function renderProjectLeaderLogCards(target_id, empty_id, id_list, loggedAtMap, hideLogBox) {
  var target_el = document.getElementById(target_id);
  var empty_el = document.getElementById(empty_id);
  if (!target_el) { return; }
  if (!id_list.length) {
  ...
  target_el.innerHTML = filtered.map(function (t) { var at = (loggedAtMap && loggedAtMap[t.treeId]) || ''; return projectLeaderLogCardHtml(t, at, hideLogBox, target_id); }).join('');
  if (!filtered.length) { target_el.innerHTML = ''; if (empty_el) empty_el.style.display = 'block'; }
}

function projectLeaderBaseData(t) {
  var q = String.fromCharCode(39);
  var c = t.card || {};
  var enc = t['encounters-list'] || {};
  var keys = Object.keys(enc);
  var last = enc[keys[keys.length - 1]] || {};
  var st = last['health-status'] || {};
  return { q:q, c:c, enc:enc, keys:keys, last:last, st:st, name_txt:projectLeaderCardName(t)||'', addr_txt:projectLeaderCardAddr(t)||c.addr||'', logs_display:keys.length||c.logs||0 };
}
function projectLeaderMyTreeCardHtml(t, is_current, hide_actions) {
  var d = projectLeaderBaseData(t);
  var added_label = t.addedAt ? (function(){ var m=/^(\d{4})(\d{2})(\d{2})T/.exec(t.addedAt); return m ? m[3]+'-'+m[2]+'-'+m[1] : t.addedAt; })() : '';
  var delete_btn = is_current ? '<button class="tcard-delete-btn" type="button" onclick="event.stopPropagation(); deleteProjectLeaderMyCurrent(' + d.q + t.treeId + d.q + ')"><i class="ti ti-trash"></i></button>' : '';
...
function projectLeaderCurrentCardHtml(t){ var h=projectLeaderMyTreeCardHtml(t,true,false); return h.replace('project-leader-my-card','project-leader-current-card'); }
function projectLeaderPastCardHtml(t){ var h=projectLeaderMyTreeCardHtml(t,false,false); return h.replace('project-leader-my-card','project-leader-past-card'); }
function projectLeaderLogCardWrapper(t,at,hide,cls){ var h=projectLeaderLogCardHtml(t,at,hide,cls); var map={ 'project-leader-logs-approved':'project-leader-log-approved-card', 'project-leader-logs-submitted':'project-leader-log-submitted-card', 'project-leader-register-log-approved':'project-leader-register-approved-card', 'project-leader-register-log-submitted':'project-leader-register-submitted-card', 'project-leader-survey-requests-approved':'project-leader-survey-approved-card', 'project-leader-survey-requests-submitted':'project-leader-survey-submitted-card' }; var c='project-leader-log-card'; for(var k in map) if(String(cls).indexOf(k)>-1) c=map[k]; return h.replace('sponsor-tree-card','sponsor-tree-card '+c); }
...
function renderProjectLeaderMyTreeCards(target_id, empty_id, id_list, addedAtMap) {
  var target_el = document.getElementById(target_id);
  var empty_el = document.getElementById(empty_id);
  if (!target_el) return;
  ...
  var fn = String(target_id).indexOf('current')>-1 ? projectLeaderCurrentCardHtml : projectLeaderPastCardHtml;
  if(String(target_id).indexOf('this-month')>-1) fn = function(t){ return projectLeaderMyTreeCardHtml(t,false,true).replace('project-leader-my-card','project-leader-month-card'); };
  target_el.innerHTML = filtered.map(function (t) { return fn(t); }).join('');
  if (!filtered.length) { target_el.innerHTML = ''; if (empty_el) empty_el.style.display = 'block'; }
}
function openProjectLeaderMyCurrent() {
  var role_cfg = getRoleConfig('project-leader');
  var tiles_my = role_cfg.tiles && role_cfg.tiles['my-trees'] || {};
  ...
  renderProjectLeaderMyTreeCards('project-leader-my-current-cards', 'project-leader-my-current-empty', current_ids, map);
  goTo('project-leader-my-current');
}

function openProjectLeaderMyPast() {
  var role_cfg = getRoleConfig('project-leader');
  var tiles_my = role_cfg.tiles && role_cfg.tiles['my-trees'] || {};
  var stats_my = role_cfg.stats && role_cfg.stats['my-trees'] || {};
  ...
  renderProjectLeaderMyTreeCards('project-leader-my-past-cards', 'project-leader-my-past-empty', past_ids, map);
  goTo('project-leader-my-past');
}
var sortDirCurrent = { time: -1, treeId: 1 };
var sortDirPast = { time: -1, treeId: 1 };
function sortProjectLeaderMyCurrent(by) {
  ...
  var ids = list.map(function(e){ return e.treeId||e; }).filter(Boolean);
  renderProjectLeaderMyTreeCards('project-leader-my-current-cards', 'project-leader-my-current-empty', ids);
}
function sortProjectLeaderMyPast(by) {
  ...
  list.sort(function(a,b){ var A=(by==='treeId'?(a.treeId||a):(a.addedAt||a.treeId||a)).toString(); var B=(by==='treeId'?(b.treeId||b):(b.addedAt||b.treeId||b)).toString(); return dir * A.localeCompare(B); });
  var ids = list.map(function(e){ return e.treeId||e; }).filter(Boolean);
  renderProjectLeaderMyTreeCards('project-leader-my-past-cards', 'project-leader-my-past-empty', ids);
}
...
function loadDashboard() {
  ...
  try { checkNewProjectLeaderTrees(); } catch (e) {}
  renderProjectLeaderStats();
  ...
  try {
    ...
    renderProjectLeaderMyTreeCards('project-leader-my-current-cards', 'project-leader-my-current-empty', current_ids, curMap);
    renderProjectLeaderMyTreeCards('project-leader-my-past-cards', 'project-leader-my-past-empty', past_ids, pastMap);
    ...
    renderProjectLeaderLogCards('project-leader-logs-approved-cards', 'project-leader-logs-approved-empty', approved_ids, mapA);
    renderProjectLeaderLogCards('project-leader-logs-submitted-cards', 'project-leader-logs-submitted-empty', submitted_ids, mapS);
  } catch (e) {}
...
if (hubMode === 'login') { window.location.href = 'login-hub.html?role=project-leader'; }
  ...
  setTimeout(function() {
    if (typeof openProjectLeaderLogsApproved === 'function') { openProjectLeaderLogsApproved(); }
    else { goTo('project-leader-logs-approved'); }
  }, 50);
}
else if (hubMode === 'project-leader-logs-submitted') {
  setTimeout(function() {
    if (typeof openProjectLeaderLogsSubmitted === 'function') { openProjectLeaderLogsSubmitted(); }
    else { goTo('project-leader-logs-submitted'); }
  }, 50);
}
else if (hubMode === 'project-leader-my-current') {
  setTimeout(function() {
    if (typeof openProjectLeaderMyCurrent === 'function') { openProjectLeaderMyCurrent(); }
    else { goTo('project-leader-my-current'); }
  }, 50);
}
else if (hubMode === 'project-leader-my-past') {
  setTimeout(function() {
    if (typeof openProjectLeaderMyPast === 'function') { openProjectLeaderMyPast(); }
    else { goTo('project-leader-my-past'); }
  }, 50);
}
else if (hubMode === 'project-leader-register-log-approved') {
  setTimeout(function() {
    if (typeof openProjectLeaderRegisterLog === 'function') { openProjectLeaderRegisterLog('approved'); }
    else { goTo('project-leader-register-log-approved'); }
  }, 50);
}
else if (hubMode === 'project-leader-register-log-submitted') {
  setTimeout(function() {
    if (typeof openProjectLeaderRegisterLog === 'function') { openProjectLeaderRegisterLog('submitted'); }
    else { goTo('project-leader-register-log-submitted'); }
  }, 50);
}
else if (hubMode === 'project-leader-tree-name-approved') {
  setTimeout(function() {
    if (typeof openProjectLeaderTreeName === 'function') { openProjectLeaderTreeName('approved'); }
    else { goTo('project-leader-tree-name-approved'); }
  }, 50);
}
else if (hubMode === 'project-leader-tree-name-submitted') {
  setTimeout(function() {
    if (typeof openProjectLeaderTreeName === 'function') { openProjectLeaderTreeName('submitted'); }
    else { goTo('project-leader-tree-name-submitted'); }
  }, 50);
}
else if (hubMode === 'project-leader-place-name-approved') {
  setTimeout(function() {
    if (typeof openProjectLeaderPlaceName === 'function') { openProjectLeaderPlaceName('approved'); }
    else { goTo('project-leader-place-name-approved'); }
  }, 50);
}
else if (hubMode === 'project-leader-place-name-submitted') {
  setTimeout(function() {
  ...
  setTimeout(function() {
    if (typeof openProjectLeaderSurveyRequests === 'function') { openProjectLeaderSurveyRequests('approved'); }
    else { goTo('project-leader-survey-requests-approved'); }
  }, 50);
}
else if (hubMode === 'project-leader-survey-requests-submitted') {
  setTimeout(function() {
    if (typeof openProjectLeaderSurveyRequests === 'function') { openProjectLeaderSurveyRequests('submitted'); }
    else { goTo('project-leader-survey-requests-submitted'); }
  }, 50);
}
