...
function continueAsRanger() {
  goTo('ranger-dash');
}

function getRangerParentUrl() {
  var parent_url = new URLSearchParams(location.search).get('parent');
  return parent_url ? parent_url : null;
}

function goBackFromRangerLogin() {
  var parent_url = getRangerParentUrl();
...
function openAppendTreeName() {
  ...
  frame.src = 'append-a-tree-name.html?parent=' + encodeURIComponent('ranger.html?hub=ranger-tree-name-submitted') + '&userid=' + encodeURIComponent(user_id);
...
function renderRangerSimpleCards(target_id, empty_id, list) {
  ...
  var html = '';
  for (var i = 0; i < list.length; i++) { html += is_tree ? rangerTreeNameCardHtml(list[i], is_submitted) : rangerPlaceNameCardHtml(list[i], is_submitted); }
...
else if (hubMode === 'register') { goTo('ranger-enroll'); }
